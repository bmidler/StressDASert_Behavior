...
while [ $retry_count -lt $max_retries ]; do
    slap-triangulate --p2d $session_directory --calib $calibration_file --fname $output_filename --scale_smooth 4 --n_deriv_smooth 2 --reproj_loss l2 --reproj_error_threshold 15
    if [ $? -eq 0 ]; then
        success=1
